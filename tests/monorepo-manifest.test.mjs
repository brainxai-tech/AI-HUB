...
test("deployment manifest covers every tool and game exactly once", async () => {
  ...
  assert.equal(manifest.projects.length, 33);
  assert.equal(manifest.games.length, 5);
  assert.equal(Object.hasOwn(manifest, "excludedGames"), false);
  assert.equal(new Set(manifest.projects.map(({ id }) => id)).size, 33);
  assert.equal(new Set(manifest.projects.map(({ route }) => route)).size, 33);
  ...
  assert.equal(new Set(allEntries.map(({ id }) => id)).size, 38);
  assert.equal(new Set(allEntries.map(({ route }) => route)).size, 38);
