...
test("HTTP API requires its optional bearer token and never caches workflow data", async (t) => {
  ...
  const runner = {
    ...
    async delete(id) {
      assert.equal(id, "sample-00000000");
      return true;
    },
  };
  ...
  const preflight = await fetch(`http://127.0.0.1:${port}/api/runs/sample-00000000`, {
    method: "OPTIONS",
  });
  assert.equal(preflight.status, 204);
  assert.match(preflight.headers.get("access-control-allow-methods") || "", /DELETE/);

  const deleted = await fetch(`http://127.0.0.1:${port}/api/runs/sample-00000000`, {
    method: "DELETE",
