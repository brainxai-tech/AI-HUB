...
const baseUrl = process.env.SMOKE_URL || "http://127.0.0.1:5198";
...
try {
  ...
  for (const viewport of viewports) {
    ...
    await page.getByRole("button", { name: "分析误解风险" }).waitFor({ timeout: 10_000 });
    await page.getByRole("button", { name: "分析误解风险" }).click();
    await page.getByRole("heading", { name: "不同人会怎么误解" }).waitFor({ timeout: 10_000 });
