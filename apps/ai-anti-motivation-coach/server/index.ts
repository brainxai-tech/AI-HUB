...
apiRouter.post("/generate", async (req, res) => {
  ...
  try {
    ...
    const isSafety = needsSafetyMode(input.userText);
    const isDemo = input.provider === "demo";
    const generated = isSafety || isDemo ? null : await generateWithProvider(input);
    const data = isSafety ? buildSafetyResult(input) : isDemo ? buildDemoResult(input) : generated!.data;
    ...
    return res.json({
      ...
      meta: {
        ...
        model: input.model,
        mode: isSafety ? "safety" : isDemo ? "demo" : "model",
...
app.listen(port, "127.0.0.1", () => {
  console.log(`AI Anti-Motivation Coach running at http://127.0.0.1:${port}${basePath || ""}`);
});
...
function handleError(res: express.Response, error: unknown) {
  ...
  console.error(error);
  return res.status(500).json(errorBody("INTERNAL_ERROR", "生成反鸡汤建议时发生未知错误。"));
}
