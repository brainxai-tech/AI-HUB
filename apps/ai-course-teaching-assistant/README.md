...
HUB_PROJECT_TOKEN=
HUB_COURSE_REQUEST_TIMEOUT_MS=190000
NEXT_PUBLIC_BASE_PATH=
```

大型教学包使用独立的超时预算：Hub 网关默认 180 秒，本项目请求默认 190 秒，Agent 工作流客户端默认 200 秒。可分别通过 `HUB_COURSE_UPSTREAM_TIMEOUT_MS`、`HUB_COURSE_REQUEST_TIMEOUT_MS` 和 `AIHUB_COURSE_TIMEOUT_MS` 覆盖；外层预算应始终大于内层预算。
