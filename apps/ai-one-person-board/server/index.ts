...
if (isMainModule()) {
  app.listen(port, "127.0.0.1", () => {
    console.log(`AI One-Person Board running at http://127.0.0.1:${port}${basePath || ""}`);
  });
}
...
function handleError(res: express.Response, error: unknown) {
  ...
  console.error(error);
  return res.status(500).json(errorBody("INTERNAL_ERROR", "生成董事会报告时发生未知错误。"));
}
