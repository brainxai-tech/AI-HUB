import { reportPlanSchema } from "../src/shared/contracts.js";
...
export const DEFAULT_PROVIDER_TIMEOUT_MS = 210_000;
export async function generateWithProvider(request, options = {}) {
    ...
    return parseReportPayload(content, request.input.slideCount);
}
export function parseReportPayload(rawText, expectedSlideCount) {
    ...
    const result = reportPlanSchema.safeParse(parsed);
    if (!result.success) {
        throw new ProviderError(502, "SCHEMA_ERROR", "GPT 型号输出缺少必要字段，请重试。", result.error.flatten());
    }
    if (expectedSlideCount && result.data.slides.length !== expectedSlideCount) {
        throw new ProviderError(502, "SLIDE_COUNT_ERROR", `模型返回了 ${result.data.slides.length} 页，而不是要求的 ${expectedSlideCount} 页，请重试。`);
    }
    return {
        ...result.data,
        slides: result.data.slides.map((slide, index) => ({ ...slide, page: index + 1 }))
    };
}
...
async function timedFetch(fetcher, url, init, timeoutMs) {
    ...
    try {
        ...
        if (error instanceof Error && error.name === "AbortError") {
            throw new ProviderError(504, "HUB_TIMEOUT", "Hub GPT 生成超时，请稍后重试或减少资料长度。");
        }
