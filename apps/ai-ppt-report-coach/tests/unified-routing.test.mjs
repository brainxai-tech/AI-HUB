...
test("Hub catalog exposes only the configured GPT route", async () => {
  ...
  const runtime = await resolveHubRuntime({
    ...
    fetcher: async (url, init) => {
      ...
      return Response.json({
        providers: [
          { id: "gemini", model: "gemini-legacy", enabled: true, configured: true },
          {
            id: "openai",
            model: "gpt-5.4",
            models: ["gpt-5.4", "not-gpt"],
  ...
  assert.equal(captured.init.headers["X-Hub-Project-Id"], "ai-ppt-report-coach");
  assert.equal(captured.init.headers["X-Hub-Project-Path"], "/ppt-report-coach");
...
test("project model selection rejects non-GPT models before contacting Hub", async () => {
  await assert.rejects(
    writeHubModelSelection("gemini-legacy", { fetcher: async () => assert.fail("fetch must not run") }),
...
test("release UI and server contain no project key controls or direct provider fallback", async () => {
  const [bundle, contracts, gateway, index] = await Promise.all([
    readFile(resolve(root, "dist/assets/index-hub-key-ec74c97443.js"), "utf8"),
  ...
  assert.doesNotMatch(bundle, /type:`password`|localStorage|apiKey|apiBaseUrl|个人 API|演示模式/i);
  assert.doesNotMatch(contracts, /apiKey|apiBaseUrl|demo/i);
